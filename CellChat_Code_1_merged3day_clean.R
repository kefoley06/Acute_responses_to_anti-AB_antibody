...
#Acute communication between microglia and peripheral immune cells in the anti-Aβ antibody injected cortex
...
object.list <- list(cc_3day_AB = cellchat_3dayAB,
                    cc_3day_IgG = cellchat_3dayIgG)

cellchat <- mergeCellChat(object.list, add.names = names(object.list))

#save(object.list, file = "cellchat_object.list_3day_ABIgG.RData")
#save(cellchat, file = "cellchat_merged_3day_ABIgG.RData")

setwd("/Users/foley/Documents/Projects/hIA/nfeature_5000/CellChat/3day_ABIgG/")
...
gg1 <- netVisual_heatmap(cellchat, color.use = color_pal)
gg2 <- netVisual_heatmap(cellchat, measure = "weight", color.use = color_pal)
gg1 + gg2
...
ggplot(IgG_scatter, aes(x = x, y = y, size = Count, label = labels))+
...
write.csv(net_infoflow_data, "netinfoflow_3day_C14_to_endos.csv")
...
#gg1 <- netVisual_bubble(cellchat, sources.use = c(11,23), targets.use = c(15,20),  comparison = c(1, 2), max.dataset = 2, title.name = "Increased signaling in Control", angle.x = 45, remove.isolate = T)
gg2 <- netVisual_bubble(cellchat, 
                        sources.use =  c(12), 
                        targets.use = c(1:23),  
                        comparison = c(1, 2), 
                        max.dataset = 1, 
                        title.name = "Increased signaling in ABinjection", angle.x = 45, remove.isolate = T)
#gg1 + gg2
gg2
...
gg2 <- netVisual_bubble(cellchat, 
                        ...
                        #signaling = c("LAMININ"),
                        max.dataset = 2, 
                        title.name = "Increased signaling in ABinjection", angle.x = 45, remove.isolate = F,
                        thresh = 0.05)
#gg1 + gg2
gg2

gg2 <- netVisual_bubble(cellchat, sources.use = c(15), targets.use = c(11,23),  comparison = c(1, 2), max.dataset = 1, title.name = "Increased signaling in 3day ABinjection", angle.x = 45, remove.isolate = T, thresh = 0.01)
gg2
...
gg4 <- netVisual_bubble(cellchat, sources.use = c(15,17,20), targets.use = c(1:23),
                        signaling = "TNF",
                        comparison = c(1, 2), max.dataset = 2, title.name = "Increased signaling in 3day IgG", angle.x = 45, remove.isolate = T, thresh = 0.01)
gg4

# define a positive dataset, i.e., the dataset with positive fold change against the other dataset
pos.dataset = "cc_3day_IgG"
...
net.up <- subsetCommunication(cellchat, net = net, datasets = "cc_3day_IgG",ligand.logFC = 0.2, receptor.logFC = NULL)
# extract the ligand-receptor pairs with upregulated ligands and upregulated recetptors in NL, i.e.,downregulated in LS
net.down <- subsetCommunication(cellchat, net = net, datasets = "cc_3day_AB",ligand.logFC = -0.1, receptor.logFC = -0.1)
...
gg1 <- netVisual_bubble(cellchat, pairLR.use = pairLR.use.up, sources.use = 12, targets.use = c(1:23), comparison = c(1, 2),  angle.x = 90, remove.isolate = T,title.name = paste0("Up-regulated signaling in ", names(object.list)[2]))
pairLR.use.down = net.down[, "interaction_name", drop = F]
gg2 <- netVisual_bubble(cellchat, pairLR.use = pairLR.use.down, sources.use = 12, targets.use = c(1:23), comparison = c(1, 2),  angle.x = 90, remove.isolate = T,title.name = paste0("Down-regulated signaling in ", names(object.list)[2]))
gg2 <- netVisual_bubble(cellchat, sources.use = c(15), targets.use = c(11,23),  comparison = c(1, 2), max.dataset = 1, title.name = "Increased signaling in 3day ABinjection", angle.x = 45, remove.isolate = F, thresh = 0.01)
gg2
gg1 + gg2
...
write.csv(gg2_test, "C14_signaling_toall.csv")

par(mfrow = c(1,2), xpd=TRUE)
netVisual_chord_gene(object.list[[2]], sources.use = c(11,23), targets.use = c(1,2,6,7,8,10,13,17), slot.name = 'net', net = net.up, lab.cex = 0.8, small.gap = 3.5, title.name = paste0("Up-regulated signaling in ", names(object.list)[2]))
netVisual_chord_gene(object.list[[1]], sources.use = c(11,23), targets.use = c(1,2,6,7,8,10,13,17), slot.name = 'net', net = net.down, lab.cex = 0.8, small.gap = 3.5, title.name = paste0("Down-regulated signaling in ", names(object.list)[2]))
...
for (i in 1:length(object.list)) {
  netVisual_aggregate(object.list[[i]], 
                      ...
                      targets.use = c(1,2,6,7,8,10,13),
                      edge.weight.max = weight.max[1], 
                      edge.width.max = 10, color.use = color_pal, 
                      signaling.name = paste(pathways.show, names(object.list)[i]))
}

cellchat@netP$cc_3day_AB$pathways
...
for (i in 1:length(object.list)) {
  netVisual_chord_gene(object.list[[i]], 
                       sources.use = c(12), 
                       targets.use = c(11,23),  
                       title.name = paste0("Signaling from OPC to endo - ", 
...
saveRDS(cellchat, file = "cellchat_comparisonAnalysis_3day_ABIgG.rds")


cellchat@meta$datasets = factor(cellchat@meta$datasets, levels = c("cc_3day_IgG", "cc_3day_AB")) # set factor level
plotGeneExpression(cellchat, signaling = "VEGF", split.by = "datasets", colors.ggplot = T, split.plot = T)



gg1 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = c("C3"))
gg2 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = c("C17"))
...
AB_weightmerged<-as.data.frame(cellchat@net$cc_3day_AB$weight.merged)
AB_countmerged<-cellchat@net$cc_3day_AB$count.merged
AB_pval<-as.data.frame(cellchat@net$cc_3day_AB$pval)
AB_weight<-as.data.frame(cellchat@net$cc_3day_AB$weight)
AB_count<-as.data.frame(cellchat@net$cc_3day_AB$count)

IgG_weightmerged<-cellchat@net$cc_3day_IgG$weight.merged
IgG_countmerged<-cellchat@net$cc_3day_IgG$count.merged
IgG_pval<-as.data.frame(cellchat@net$cc_3day_IgG$pval)
IgG_weight<-as.data.frame(cellchat@net$cc_3day_IgG$weight)
IgG_count<-as.data.frame(cellchat@net$cc_3day_IgG$count)

write.csv(AB_weightmerged, file = "3day_AB_weightmerge.csv")
write.csv(AB_countmerged, file = "3day_AB_countmerge.csv")
write.csv(IgG_weightmerged, file = "3day_IgG_weightmerge.csv")
write.csv(IgG_countmerged, file = "3day_IgG_countmerge.csv")

write.csv(AB_pval, "3day_AB_pval.csv")

write.csv(AB_weight, file = "3day_AB_weight.csv")
write.csv(AB_count, file = "3day_AB_count.csv")
write.csv(IgG_weight, file = "3day_IgG_weight.csv")
write.csv(IgG_count, file = "3day_IgG_count.csv")
